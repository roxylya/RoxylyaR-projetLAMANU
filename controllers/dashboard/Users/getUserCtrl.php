            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if (User::existsEmail($email) === true && $email != $user->email) {
                    ...
                    if ($userUpdate->updateEmail($id_users) === true) {
                        ...
                        // mail de validation :
                        $link = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . '/controllers/validateMailCtrl.php?id_users=' . $theUser->id_users;
                        $for = $theUser->email;
                        ...
                        $message = 'Le changement d\'email a été enregistré.';
            ...
            if (!filter_var($pseudo, FILTER_VALIDATE_REGEXP, array("options" => array("regexp" => '/' . REGEX_PSEUDO . '/')))) {
                ...
                if (User::existsPseudo($pseudo) === true && $pseudo != $theUser->pseudo) {
                    ...
                    if ($userUpdate->updatePseudo($id_users) === true) {
                        $message = 'Le changement de  pseudo a été enregistré.';
                        Session::setMessage($message);
                        Session::getMessage();
                    }
                }
        ...
        if (isset($_FILES['avatar'])) {
            ...
            if (!empty($avatar)) {
                if (!in_array($avatarType, EXTENSION)) {
                    ...
                    $oldAvatar = __DIR__ . '/../../public/uploads/avatars/avatar_' . $theUser->id_users . '.' . $theUser->extUserAvatar;
                    ...
                    if ($userUpdate->updateExtUserAvatar($id_users) === true) {
                        $message = 'Le changement de l\' avatar a été enregistré.';
                        Session::setMessage($message);
                        Session::getMessage();
                        $avatarName = 'avatar_' . $theUser->id_users . '.' . $extUserAvatar;
...
include(__DIR__ . '/../../../views/dashboard/User/getUser.php');
