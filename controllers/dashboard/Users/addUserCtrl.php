...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        if (empty($error)) {
            ...
            $newUser->setExtUserAvatar($extUserAvatar);
            $newUser->setId_roles($id_roles);
            // Ajouter l'enregistrement du nouveau user à la base de données :
            if ($newUser->add() === true) {

                // je sauvegarde l'avatar :
                $newUser = User::getByEmail($email);
                $avatarName = 'avatar_' . $newUser->id_users . '.' . $extUserAvatar;
                ...
                // mail de validation :
                $link = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'] . '/controllers/validateMailCtrl.php?id_users=' . $newUser->id_users;
                $for = $user->email;
...
include(__DIR__ . '/../../../views/dashboard/User/addUser.php');
